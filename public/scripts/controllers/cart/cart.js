...
const controlRemoveCartProduct = async (uuid) => {
  ...
  const {
    data: { total },
    error,
  } = await shopModel.getCartProductsTotalPrice();
...
const controlQuantityChange = async (uuid, quantity) => {
  const { data, error } = await shopModel.updateCartProductQuantity(
    uuid,
    quantity
  );
  ...
  const { data: totalPriceData, error: totalPriceError } =
    await shopModel.getCartProductsTotalPrice();

  if (totalPriceError)
    controlRenderMessage(ORDER_TOTAL_NOT_ACCURATE, MESSAGE.MESSAGE_ERROR, LONG);

  if (!totalPriceError)
    CartView.updateTotalPrice(totalPriceData.total.toFixed(2));
...
const controlUpdateCartProduct = async (uuid) => {
  ...
  const { data, error } = await shopModel.updateCartProduct(
    uuid,
    productData.tiers,
    productData
  );
  ...
  for (const ingredient of cartProduct.ingredients) {
    for (let tiersIndex = 0; tiersIndex < tiersIDS.length; tiersIndex += 1) {
      if (tiersIDS[tiersIndex] === ingredient.split(".")[0]) {
        product.tiers[tiersIndex].selectedIngredients.push(
          ingredient.split(".")[1]
        );
...
export const controlRenderCart = async () => {
  ...
  if (!CartView._rerender) {
    CartView.render({
      ...
      itemMethods: {
        onClick: (uuid) => {
          controlRenderEditCartProduct(uuid);
        },
        remove: (uuid) => {
          controlRemoveCartProduct(uuid);
        },
        quantityChange: (uuid, quantity) => {
          return controlQuantityChange(uuid, quantity);
        },
      },
      methods: {
        onOrder: () => {
          orderController.controlRenderOrderInfo();
        },
      },
    });
  }
