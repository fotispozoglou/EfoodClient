...
const getSuccessfulElement = (message) => {
  const icon = new DOMElement("i")
    .setClass("fas fa-check-circle success_item_edit_icon")
    .getElement();

  const messageElement = new DOMElement("p")
    .setText(message)
    .setClass("success_item_edit_message")
    .getElement();

  return new DOMElement("div")
    .setClass("success_item_edit")
    .append(icon, messageElement)
    .getElement();
};
...
export default new (class ProductPreferences extends EditItemView {
  ...
  _generateFooter() {
    ...
    quantityElement.onChange((value) => {
      this._priceElement.textContent = `${(value * price).toFixed(2)}€`;
    });
    ...
    return new DOMElement("div")
      .setClass("edit_item_footer")
      .append(quantityElement.build(), ...actions)
      .getElement();
  }
  ...
  _generateHeader() {
    ...
    const backBtn = new DOMElement("i")
      .setClass("fas fa-arrow-left edit_item_header_action")
      .on("click", () => {
    ...
    const { name, price, quantity } = this._data.item;

    const title = new DOMElement("p")
      .setClass("product_preferences_title")
      .setText(name)
      .getElement();

    this._priceElement = new DOMElement("p")
      .setClass("product_preferences_price")
      .setText(`${(price * quantity).toFixed(2)}€`)
      .getElement();

    const productInfoContainer = new DOMElement("div")
      .setClass("product_info_container")
      .append(title, this._priceElement)
      .getElement();
  ...
  _generateInputs() {
    ...
    for (const tier of tiers) {
      const tierElement = new SelectionsElement(
        tier.name,
        tier._id,
        tier.type,
        tier.ingredients,
        tier.selectedIngredients,
        tier.maxSelections,
        () => {}
      );
    ...
    const commentsLabel = new DOMElement("label")
      .setClass("comments_label")
      .setText(COMMENTS)
      .getElement();
    ...
    const commentsContainer = new DOMElement("div")
      .setClass("comments_container")
      .append(commentsLabel, commentsElements)
      .getElement();
    ...
    this.addDataElements(
      [
        "tiers",
        () => {
          return tiersElements.map((te) => {
            return {
              _id: te._id,
              selected: te.getSelected(),
              ingredients: te._selections,
            };
          });
        },
      ],
      [
        "comments",
        () => {
          return commentsElements.value;
        },
      ]
    );

    return [
      ...tiersElements.map((te) => {
        return te.build();
      }),
      commentsContainer,
    ];
